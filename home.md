...
Command-line, open source networking utility used for reading from and writing to network connections using TCP or UDP protocols. In particular, during this simulation it has been used to establish a local listener on the attacker's machine and capture the connection from the victim in order to begin a Reverse Shell session.
...
![Security Level](images/secLevel.png)
...
Now, in order to connect to the listening Server previously opened, the `cmd` parameter content has to be changed in:
